...
|---|-----------|---------------|--------------|
| M1 | Project Setup & Infrastructure | 1–2 | 5 |
...
**What**: Bootstrap the admin web app — hosts the Super Admin dashboard and the Venue subscription page.

| Sub-task | Details |
|----------|---------|
| Next.js init | TypeScript + App Router in `apps/admin` |
| ShadCN/UI | Install and configure component library |
| Route structure | `/login`, `/dashboard`, `/users`, `/events/pending`, `/subscribe` |
| Layout | Sidebar nav (Dashboard \| Users \| Pending Events) + header with logout |
| Smoke test | Dev server runs, ShadCN components render |
...
| Sub-task | Details |
|----------|---------|
| `/account` route | In Next.js admin app — requires CeolX login before access |
