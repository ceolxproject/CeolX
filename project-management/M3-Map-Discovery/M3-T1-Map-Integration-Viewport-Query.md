...
| **Status** | 🔲 To Do |
| **Depends on** | M1-T2 (events table + GIST index), M1-T3 (API scaffold), M1-T4 (mobile scaffold), M4-T1 (events must exist to test meaningfully) |
...
The main screen of the CeolX app — the Map view. Users see event pins scattered across an interactive map at real-time locations. Performance is critical: only fetch events visible in the current viewport using a bounding box query (SW + NE lat/lng corners), capped at 50 pins per fetch. As the user pans, new requests fire on a ~400ms debounce after they stop moving. This prevents excessive API load while keeping the map responsive and always fresh.

The map renders platform-specific: Apple Maps on iOS, Google Maps on Android. Both use the same React Native Maps component. Pins render at precise lat/lng coordinates with category labels. Tapping a pin opens an Event Detail bottom sheet with quick info and a "See full details" expansion option.
