...
```typescript
import { uuid, text, timestamp, pgEnum, pgTable, uniqueIndex, foreignKey } from 'drizzle-orm/pg-core';

export const bookingStatusEnum = pgEnum('booking_status', ['pending', 'accepted', 'rejected', 'cancelled']);
export const bookingDirectionEnum = pgEnum('booking_direction', ['venue_to_artist', 'artist_to_venue']);

export const bookings = pgTable(
  'bookings',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    artist_id: uuid('artist_id').notNull().references(() => artistProfiles.id),
    venue_id: uuid('venue_id').notNull().references(() => venueProfiles.id),
  ...
  (table) => ({
    uniqueBooking: uniqueIndex('idx_booking_artist_event_active').on(
      table.artist_id,
      table.event_id,
...
bookingsRouter.patch('/:id', async (c) => {
  ...
  const isVenue = user.current_role === 'venue' &&
