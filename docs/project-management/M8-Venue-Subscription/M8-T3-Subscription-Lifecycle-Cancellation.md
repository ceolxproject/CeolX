...
- R2.1: On `invoice.payment_failed` webhook:
  ...
  - Send Postmark failure email: *"Payment failed. Your subscription is paused."* + retry instructions + link to `ceolx.ie/account` to update payment method
...
- R3.1: On `customer.subscription.deleted` webhook:
  ...
  - Send Postmark cancellation confirmation email: *"Your subscription has been cancelled."* + offer to reactivate
...
- Body: *"Hi [VenueName], Your recent payment failed due to an issue with your payment method. Your subscription is temporarily paused. [Button: Update Payment Method] → ceolx.ie/account"*
...
- Subject: *"Your CeolX Subscription Has Been Cancelled"*
- Body: *"Hi [VenueName], Your subscription has been cancelled. Your profile is no longer visible to artists. [Button: Reactivate] → ceolx.ie/subscribe"*
