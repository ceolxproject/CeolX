...
- R3: **Fallback Step 2 — IP Geolocation**: permission denied → call `GET /location/ip` → centre map on returned city/county; show subtle banner: _"Using approximate location — search to refine."_
...
- R8: If still 0 results after 100 km retry → show a non-blocking floating card over the map: _"No events near here. Try searching for Dublin, Galway, or Cork."_ + **Browse all upcoming events** button (switches to Discover tab)
