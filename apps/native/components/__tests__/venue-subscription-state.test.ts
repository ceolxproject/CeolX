...
describe('venueStateFor — gate-aware, not status-inferred', () => {
  it('shows nothing for an inactive venue while the gate is off', () => {
    // The shipping default. This is the case that would have mislabelled every venue.
    expect(venueStateFor({ status: 'inactive', onHold: false })).toBe('none');
    expect(venueStateFor({ status: 'cancelled', onHold: false })).toBe('none');
  });
