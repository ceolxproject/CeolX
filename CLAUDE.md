...
4. In-app shows: *"Your profile is not yet visible to artists. Check your email to activate."* + **Resend Email** button
5. **No external URL is shown inside the app** — this avoids App Store rejection
6. Venue opens email → clicks link → `ceolx.ie/subscribe` (hosted in Next.js project)
...
2. **IP geolocation** (denied) → server resolves approximate city/county from IP. Shows banner: *"Using approximate location — search to refine."*
...
4. If still 0 → show non-blocking floating card: *"No events near here. Try searching for Dublin, Galway, or Cork."* + **Browse all upcoming events** (switches to Feed view)
...
| Admin Dashboard | Next.js + ShadCN/UI | Also hosts ceolx.ie/subscribe page |
...
apps/
  ...
  admin/      -- Next.js + ShadCN (also hosts ceolx.ie/subscribe)
